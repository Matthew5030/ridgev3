...
The new atlas journey holds terrain at 4 m and rejects oversized selections instead of silently lowering detail. Surroundings are included when available and admitted. The adaptive terrain format remains a separate experiment. Current source data is bundled or imported, so the UI says no download is needed; outside complete prepared coverage it explains the limitation instead of inventing a hosted download. Saved areas are shaded on the atlas. Identical independent map images are shared between overlapping saved areas, while terrain crops and graphs remain separate. Removing one area does not remove maps used by another.
