...
xcrun swiftc -O -swift-version 6 -parse-as-library \
  ...
  "$RIDGE_PROJECT_ROOT/Tools/ParkSourcePerformance.swift" \
  -o "$RIDGE_TEST_BUILD/ParkSourcePerformance"
"$RIDGE_TEST_BUILD/ParkSourcePerformance" "${1:?Pass the prepared park source directory}" "${2:-}"
