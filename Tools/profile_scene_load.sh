...
RIDGE_PROFILE_BUILD=$(mktemp -d "${TMPDIR:-/tmp}/ridge-load-profile.XXXXXX")
...
xcrun swiftc -O -swift-version 5 -parse-as-library \
  ...
  "$RIDGE_PROJECT_ROOT/ridge v3/Terrain/CartographyRenderer.swift" \
  "$RIDGE_PROJECT_ROOT/Tools/SceneLoadProfile.swift" \
  -o "$RIDGE_PROFILE_BUILD/SceneLoadProfile"
"$RIDGE_PROFILE_BUILD/SceneLoadProfile" "$RIDGE_PROJECT_ROOT/ridge v3/Resources/RidgeData.bundle" "$@"
