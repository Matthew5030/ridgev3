...
p.add_argument('--ea-coverage',type=Path,required=True);p.add_argument('--id',default='uk-adaptive-0p5-v2')
a=p.parse_args()
...
survey=SurveyCoverage(a.ea_coverage)
print('Verified official survey footprints:',survey.provenance['featureCount'],flush=True)
def scan(tile):
 b=tile['bounds'];area=uk.intersection(bbox(b));selected={};manifests=[];rejected=set();by_source={};ea_area=None
 for root in roots:
 ...
 d=dict(id=tile['tileID'],x=tile['x'],y=tile['y'],bounds=b,chunkCount=len(selected),sourceBytes=sum(c['sourceByteCount'] for c in selected.values()),coveredPolygonKm2=transform(project,covered).area/1e6,polygonKm2=transform(project,area).area/1e6,sourceManifests=manifests,rejectedEACandidates=len(rejected),rejectedWithoutFallback=len(rejected-set(selected)),selectedBySource=by_source,chunks=list(selected.values()))
